...
if (!class_exists("nxs_class_SNAP_IG")) { class nxs_class_SNAP_IG {
    ...
    function doPostToNT($options, $message){ $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>''); if (!class_exists("nxsAPI_IG")){ $badOut['Error'] .= "Instagram API not found"; return $badOut; } 
      ...
      //prr($options['ck']); die();
      $nt = new nxsAPI_IG(); $nt->debug = false; $nt->opNm = $opNm;  if(!empty($options['ck'])) $nt->ck = $options['ck']; 
      if (!empty($options['proxy'])&&!empty($options['proxyOn'])){ $nt->proxy['proxy'] = $options['proxy']['proxy']; if (!empty($options['proxy']['up'])) $nt->proxy['up'] = $options['proxy']['up'];}; //     prr($_POST); die();
